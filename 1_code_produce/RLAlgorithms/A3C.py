...
from ray import tune #for clean logging 
from ray.rllib.algorithms.a3c import A3CConfig #config builder for the algorithm (which env, how many actors, learning rate, ... )
...
#for extensive comments check out APEXDQN.py (same code structure)
...
if __name__ == "__main__":
    ...
    output_directory = os.path.join(PROJECT_ROOT, "RLAlgorithms", "a3c_runs")
    ...
    config = (
        ...
        .environment(
            ...
            env_config = {
                ...
                "algo_name": "a3c", #dynamic routes are True per default in the environment
            },
        )
        ...
        .training(
            ...
            gamma = 0.99, #discount factor in V(s), paper 
            lr = 1e-4, #learning rate for each worker, paper 
            train_batch_size = 500, #paper 
            model = {"fcnet_hiddens": [128], "fcnet_activation": "tanh"} #paper 
            #no other parameter is stated in the paper, hence we assume default values 
        )
    )
    ...
    target_timesteps = 600_000_000 #600 mio target steps 
    prev_timesteps = 0

    logging_path = os.path.join(output_directory, "train_log_a3c.jsonl")
    ...
    progress_bar = tqdm(total = target_timesteps, desc = "A3C Training", unit = "ts", dynamic_ncols=True, position = 0, leave=True)
    live_plot = LivePlot(save_path=os.path.join(output_directory, "a3c_live_reward.png"), algo_name="a3c")
    ...
    ray.shutdown() #kill all processes when done with training